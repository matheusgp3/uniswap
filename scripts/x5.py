...
class uniswap:
    ...
    def __init__(self,token0,token1,qntd_token0,preco_Min,preco_max):
        self.token0 = token0
        self.token1 = token1
    ...
    def metamask(self):
        ...
        res = self.getSecretPhrase()
        mySecretPhrase = res['phrase']
        ...
        driver.find_element(by=By.ID,value="password").send_keys(myPassword)
        driver.find_element(by=By.ID,value="confirm-password").send_keys(myPassword)
    ...
    def creatingPool(self):
        ...
        driver.find_element(by=By.ID,value="token-search-input").send_keys(self.token0)
        sleep(2)
        self.checkToken(self.token0)

        # selecionando token secundario
        elemento_Btn[1].click()
        driver.find_element(by=By.ID,value="token-search-input").send_keys(self.token1)
        sleep(2)
        ...
        driver.find_element(by=By.CSS_SELECTOR,value=".token-amount-input").send_keys(self.qntd_token0)
...
f = uniswap(token0 = "MATIC",token1 = "AAVE",qntd_token0=10,preco_Min=0,preco_max=0.1)
f.main()